...
EXAMPLES = {
    "dummy-dist": {
        ...
        "licenses": {"DUMMYFILE": ""},
        **dict.fromkeys(DEFAULT__FILES | OTHER_IGNORED_FILES, ""),
    },
    "simple-dist": {
        "setup.py": cleandoc(
            """
...
def test_licenses_default(dummy_dist, monkeypatch, tmp_path):
    ...
    with ZipFile("dist/dummy_dist-1.0-py3-none-any.whl") as wf:
        license_files = {
            "dummy_dist-1.0.dist-info/" + fname for fname in DEFAULT__FILES
        }
    ...
    [
        ...
        (
            ...
            SETUPPY_EXAMPLE.replace(
                ")", "  license_files=['licenses/DUMMYFILE', ''])"
            ),
        ),
    ],
)
def test_licenses_override(dummy_dist, monkeypatch, tmp_path, config_file, config):
    ...
    with ZipFile("dist/dummy_dist-1.0-py3-none-any.whl") as wf:
        license_files = {
            "dummy_dist-1.0.dist-info/" + fname for fname in {"DUMMYFILE", ""}
        }
        assert set(wf.namelist()) == DEFAULT_FILES | license_files
