...
def _pep621_example_project(
    ...
    (tmp_path / ".txt").write_text("---  stub ---", encoding="utf-8")
...
def test_pep621_example(tmp_path):
    ...
    assert dist.metadata.license == "---  stub ---"
    assert set(dist.metadata.license_files) == {".txt"}
...
class TestLicenseFiles:
    ...
    def base_pyproject(self, tmp_path, additional_text):
        ...
        # Sanity-check
        assert 'license = {file = ".txt"}' in text
    ...
    def test_default_patterns(self, tmp_path):
        ...
        dist = pyprojecttoml.apply_configuration(makedist(tmp_path), pyproject)
        assert (tmp_path / ".txt").exists()  # from base example
        assert set(dist.metadata.license_files) == {*license_files, ".txt"}
