...
class TestEggInfo:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                True,
            ),  # no license_file attribute,  auto-included
            (
                {
                    'setup.cfg': DALS(
                        """
    ...
    def test_setup_cfg_license_file(self, tmpdir_cwd, env, files, license_in_sources):
        ...
        if license_in_sources:
            assert '' in sources_text
        else:
            assert '' not in sources_text
            # for invalid license test
            assert 'INVALID_' not in sources_text
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                [''],
                [],
            ),  # no license_files attribute,  auto-included
            (
                {
                    'setup.cfg': DALS(
                        """
    ...
    def test_license_file_attr_pkg_info(self, tmpdir_cwd, env):
        ...
        # Only 'NOTICE', -ABC', and '-XYZ' should have been matched
        # Also assert that order from license_files is keeped
        assert "License-File: NOTICE" == license_file_lines[0]
        assert "License-File: -ABC" in license_file_lines[1:]
        assert "License-File: -XYZ" in license_file_lines[1:]
